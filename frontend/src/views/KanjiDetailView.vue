      ...
      <div v-else class="kanji-detail">
        ...
        <header class="detail-header">
          ...
          <div class="header-left">
            <!-- Player stroke animCJK -->
            <StrokePlayer :character="kanji.kanjiCharacter" type="kanji" />
          </div>
              ...
              <div class="components-list">
                <button v-for="comp in kanji.components" :key="comp.radId"
                  ...
                  <div class="comp-info" v-if="comp.radMeaningEnglish || comp.radNameRomaji">
                    ...
                    <span class="comp-meaning" v-if="comp.radMeaningEnglish">{{ comp.radMeaningEnglish }}</span>
                  </div>
                </button>
              </div>
            </div>
            <div v-else class="components-missing">
              <span>Radicaux non disponibles pour ce kanji</span>
            </div>
          </div>
        </header>
        ...
        <section class="detail-section">
          ...
          <div v-else>
            <div class="words-grid">
              <RouterLink v-for="w in words" :key="w.wordId" :to="`/mots/${w.wordId}`" class="word-card">
...
import StrokePlayer from '../components/StrokePlayer.vue'
...
function gradeInfo(grade) {
  ...
  if (grade === 8) return {
    ...
    description: 'Kanji appris au collège et lycée japonais. Avec les 1006 du primaire, ils forment les 2136 jōyō kanji.',
  }
  if (grade === 9) return {
    label: 'Prénom',
    badgeClass: 'badge-grade-prenoms',
    title: '人名用漢字 · Jinmeiyō kanji (grade 9)',
    description: 'Kanji autorisé pour les prénoms japonais, hors programme scolaire standard.',
  }
...
.tooltip-box { display: none; position: absolute; bottom: calc(100% + 8px); left: 50%; transform: translateX(-50%); background: var(--ink); color: var(--paper); font-family: var(--font-body); font-size: 0.78rem; line-height: 1.6; padding: 0.75rem 1rem; border-radius: var(--radius); width: 260px; z-index: 100; box-shadow: var(--shadow-lg); pointer-events: none; text-align: left; white-space: normal; }
...
.comp-char { font-family: var(--font-cjk, 'Noto Sans JP', sans-serif); font-size: 1.4rem; line-height: 1; color: var(--vermilion); }
